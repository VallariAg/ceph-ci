...
void MDSRankDispatcher::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
  ...
  if (changed.count("mds_extraordinary_events_dump_interval")) {
    ...
    if (extraordinary_events_dump_interval) {
      if (log_level < 10) {
        if (gather_level < 10) {
          //if gather level is < 10, bump the gather level (>=10) to enable gathering in-memory logs
          g_conf().set_val("debug_mds", static_cast<std::ostringstream&&>
                           (std::ostringstream{} << log_level << '/' << 10).str());
        }
        ...
        dout(0) << __func__ << " Enabling in-memory log dump failed. Log level=" << log_level << dendl;
